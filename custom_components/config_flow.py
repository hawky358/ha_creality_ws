...
from .const import (
    DOMAIN, CONF_HOST, CONF_NAME, DEFAULT_NAME, WS_PORT,
)
...
async def _probe_tcp(host: str, port: int, timeout: float = 2.5) -> bool:
    """Fast non-blocking TCP connect probe; returns True if connect succeeds."""
    try:
...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Config flow for Creality WebSocket printers."""
    VERSION = 3
